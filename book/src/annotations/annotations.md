...
roo has a small, simplified version of Rust's attribute system: `#[...]`,
...
`#[derive(...)]` and `#[repr(C)]` — roo has none of that (see
[Differences from Rust](../design/differences-from-rust.md)). An
annotation in roo is inert as far as the roo compiler is concerned: it's
structured data attached to a declaration, nothing more. roo itself never
inspects, expands, or acts on an annotation's contents. All meaning comes
from *outside* the script — from the host embedding roo — which is the
...
roo deliberately doesn't ship a fixed vocabulary of built-in annotations
the way Rust ships `derive`, `cfg`, and `repr`. Instead, the host
embedding roo — for [roo-engine](https://github.com/LucaMezz/roo-engine),
...
its own API surface rather than roo hard-coding one.
...
with an asset to play. None of that is roo's concern — the language's job
...
roo doesn't mandate *when* the host reads annotations, either — that's a
property of the embedding, not the language. A reasonable implementation
might inspect them once, at load time (roo-engine, for instance, would do
...
the declaration immediately after it. roo's grammar also has room for an
...
positions accept `#![...]` in roo, and what it means there, isn't settled
