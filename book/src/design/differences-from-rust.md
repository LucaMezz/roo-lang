...
This page is the canonical list of every place roo's design departs from
...
These concepts have no equivalent in roo, because they exist in Rust
specifically to manage memory and hardware without a garbage collector or
runtime, a job roo's runtime does for you.

| Rust concept | Status in roo |
|---|---|
...
| Lifetimes (`'a`, `'static`, lifetime elision) | Removed entirely, including from loop labels — see [Loops](../control-flow/loops.md) for roo's non-lifetime-shaped label syntax. |
| `unsafe` blocks/functions/traits | Removed. There is no operation in roo that is memory-unsafe. |
| Raw pointers (`*const T`, `*mut T`) | Removed. |
| Manual memory management, custom/explicit allocators (`Box::new` as allocation, `#[global_allocator]`) | Removed. All reference-typed values are managed by the roo runtime automatically. |
| `extern`, `#[no_mangle]`, `#[repr(C)]`, and other FFI machinery | Removed. roo has no C ABI story at the language level. Instead, a host embedding roo exposes native functionality as an [ambient module](../modules/ambient-modules.md) — an ordinary roo module whose function bodies are simply omitted. |
| `crate` keyword, `crate::` path root, `pub(crate)` | Removed. roo has modules, but nothing sits above the module tree the way a crate sits above modules in Rust — a roo program *is* its module tree, resolved once, in full, at compile time. See [Modules and Visibility](../modules/modules.md). |
...
| `Rc<T>`, `Arc<T>`, `RefCell<T>`, `Cell<T>`, `Mutex<T>`, `Box<T>` as a distinct owning pointer | Removed as language concepts. Every reference type in roo already behaves like a shared, mutable, runtime-managed pointer, so none of these wrapper types are needed to get that behavior. |
| `static` items, interior mutability of globals | Removed, with no direct module-scoped replacement — roo has no module-level `let` either (see [Variables: No module-level bindings](../bindings/variables.md#no-module-level-bindings)). Shared mutable state has to be threaded through explicitly (function arguments, closures) rather than named at module scope. |
| `const` | Removed, with no direct replacement — see [Variables: No module-level constants, for now](../bindings/variables.md#no-module-level-constants-for-now) for why, and the `pub fn` pattern that covers the same need today. |
...
These concepts exist in roo, but in a smaller form than Rust's, because the
...
Two things roo has that Rust does not:
...
- **Ambient modules.** A module made of bodyless function signatures,
  describing an API a host embedding roo provides to scripts, used with
  ...
  roo removes. See [Ambient Modules](../modules/ambient-modules.md).
