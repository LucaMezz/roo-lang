...
| `macro_rules!`, procedural/derive macros, `#[attribute]`s in general | Removed. There is no macro system and no attribute syntax. See [Structs](../data-types/structs.md) and [Enums](../data-types/enums.md) for how the most common derive use cases (equality, display) are handled instead. |
