# Summary

[Introduction](introduction.md)

# Design

- [Philosophy](design/philosophy.md)
- [The Value Model](design/values-and-mutation.md)
- [Differences from Rust](design/differences-from-rust.md)

# Lexical Structure

- [Comments](lexical/comments.md)
- [Identifiers and Keywords](lexical/identifiers-and-keywords.md)
- [Literals](lexical/literals.md)

# Types

- [Type System Overview](types/overview.md)
- [Gradual Typing](types/gradual-typing.md)
- [Primitive Types](types/primitives.md)
- [Strings and Characters](types/strings-and-chars.md)
- [Arrays and Tuples](types/arrays-and-tuples.md)
- [Type Inference](types/inference.md)
- [Casting and Conversion](types/casting.md)

# Variables and Bindings

- [Variables](bindings/variables.md)

# Expressions and Operators

- [Expressions and Statements](expressions/expressions-and-statements.md)
- [Operators](expressions/operators.md)
- [Blocks and Scope](expressions/blocks.md)

# Control Flow

- [if and if let](control-flow/if.md)
- [Loops](control-flow/loops.md)
- [match](control-flow/match.md)

# Functions and Closures

- [Functions](functions/functions.md)
- [Closures](functions/closures.md)

# Custom Data Types

- [Structs](data-types/structs.md)
- [Enums](data-types/enums.md)
- [Pattern Matching](data-types/pattern-matching.md)

# Abstraction

- [Traits](abstraction/traits.md)
- [Generics](abstraction/generics.md)
- [Operator Overloading](abstraction/operator-overloading.md)

# Program Organization

- [Modules and Visibility](modules/modules.md)
- [Ambient Modules](modules/ambient-modules.md)

# Error Handling

- [Result, Option, and the ? Operator](errors/error-handling.md)
- [Panics](errors/panics.md)

# Appendix

- [Keyword Reference](appendix/keywords.md)
- [Operator Precedence](appendix/operator-precedence.md)
- [Grammar Summary](appendix/grammar.md)
