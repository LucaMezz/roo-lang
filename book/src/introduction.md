...
roo is an embeddable, gradually-typed scripting language. Syntactically and
semantically, it is a subset of [Rust](https://www.rust-lang.org/): if you
know Rust, you already know most of roo, and almost everything that is legal
roo source is legal to *read* as if it were Rust.

roo keeps the parts of Rust that make programs easy to reason about —
...
On top of that smaller core, roo adds one thing Rust does not have: type
annotations are optional almost everywhere. Write one and the compiler
checks it; leave it off and roo **infers** the strongest type it can —
...
wherever nothing's annotated — roo only falls back that far in the
...
for why roo ended up here. Either way, a script can start as loose,
...
This book is the language specification and reference guide for roo. It
documents the *syntax and semantics of the language itself* — every kind of
statement, expression, declaration, and type that roo understands. It does
**not** document a standard library, because roo does not have one yet. Where
...
language-level contract roo guarantees and leaves the concrete library
design as future work.

## Project status

roo is early. As of this writing, the
[roo-lang](https://github.com/LucaMezz/roo-lang) repository has a working
...
roo is being developed alongside
[roo-engine](https://github.com/LucaMezz/roo-engine), a game engine, which it
...
sample. Code blocks are labeled `roo`, not `rust` — the syntax is very close,
but treat every example as roo source, not Rust source, since small
