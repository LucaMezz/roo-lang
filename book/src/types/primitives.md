...
roo has four primitive types. All four are **value types** — see
...
roo collapses all of that to a single `int` type, because choosing an exact
width is precisely the kind of low-level, memory-layout concern roo omits —
...
roo does not have a separate `f32` — see
...
roo has no implicit zero-initialization — every `let` binding requires
